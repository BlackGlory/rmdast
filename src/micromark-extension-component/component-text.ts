...
export function componentText(): Construct {
  return {
    name: 'htmlText'

    /**
     * code是由micromark的preprocessor生产的一种代表字符的数字, 特殊值null代表EOF.
     *
     * type State: (code: Code) => State | void
     * State是一种用于处理连续code的解析函数,
     * 每处理一个code, 就会返回一个新的State, 用以处理下一个code, 直到不再返回State.
     *
     * @param effects
     * micromark里的事件机制, 具有effects.enter, effects.consume, effects.exit等方法
     *
     * @param ok {State}
     * 作为成功状态下的返回值使用, 示例: `return ok`
     *
     * @param nok {State}
     * 作为不成功状态下的返回值使用, 需要退回当前code, 示例: `return nok(code)`
     *
     * @returns {State}
     */
